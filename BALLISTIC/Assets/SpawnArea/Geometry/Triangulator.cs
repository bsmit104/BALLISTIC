                    ...
                    if (p0.x > holeData.bridgePoint.x || p1.x > holeData.bridgePoint.x)
                    {
                        ...
                        if (p0.y > holeData.bridgePoint.y != p1.y > holeData.bridgePoint.y)
                        {
                            ...
                            if (rayIntersectX > holeData.bridgePoint.x)
                            {
                                ...
                                float prevY;
                                if (potentialNewBridgeNode == vertexList.First) {
                                    prevY = vertexList.Last.Value.position.y;
                                } else {
                                    prevY = potentialNewBridgeNode.Previous.Value.position.y;
                                }
								bool connectToThisDuplicateEdge = holeData.bridgePoint.y > prevY;
